...
def upload_file():
    if request.method == 'POST':
        ...
        artist = dict(request.values)['artists'] # get the input of desired artist

        if artist =='vincent-van-gogh':
            image = load_image(file)
            image_generated = paint_generator(model_vangogh, image, 'generated_'+file.filename)
            image_file_path = f'/static/img/generated_{file.filename}'   
        if artist =='claude-monet':
            image = load_image(file)
            image_generated = paint_generator(model_monet, image, 'generated_'+file.filename) 
            image_file_path = f'/static/img/generated_{file.filename}'
        ...
        #                             filename=filename))
        return render_template('results.html', image_file_path= image_file_path, artist = artist)
...
#     return render_template('results.html', user_number = user_number, recommendation_list = recommendation_list)    
...
#     # search_results = utils.search_movie(query, datasets.movies)
#     return render_template('results.html', dict_list = dict_list, recommendation_list = recommendation_list)    
...
#     #print(movie_poster)
#     return render_template('movie.html', movie=movie, movie_id=movie_id, movie_poster = movie_poster)
