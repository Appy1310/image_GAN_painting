...
class MyFancyModelRecommender:
    ...
    def user_recommendation_internal(
            self, user_number, number_of_recommendation=5):
        # array for movies
        m_id = list(ratings.movieId.unique())   # list of all unique movie id's
        movie_arr = np.array(m_id)  # get all movie IDs
        item_enc = LabelEncoder()
        # transfor upto number of unique movie id's
        movie_arr_en = item_enc.fit_transform(movie_arr)
        # array for user
        ...
        recommendation = pd.DataFrame(
            {
                ...
                'movieId': ratings.movieId.unique()}).sort_values(
            by=['predicted_rating'],
            ascending=False)
    ...
    def user_recommendation_external(
            self,
            user_input_movies,
            number_of_recommendation=5):
        # Get the indexes of movies entered by user
        user_mov_index = [process.extractOne(movie, movies['title'])[
            2] for movie in user_input_movies]
        # array for movies
        m_id = list(ratings.movieId.unique())   # list of all unique movie id's
        interacted_items = user_mov_index
        not_interacted_items = set(m_id) - set(interacted_items)
        selected_not_interacted = np.random.choice(
            list(not_interacted_items), len(m_id) - len(user_mov_index))
        movie_arr = np.concatenate(
            (np.array(interacted_items), selected_not_interacted), axis=None)
        item_enc = LabelEncoder()
        # transfor upto number of unique movie id's
        movie_arr_en = item_enc.fit_transform(movie_arr)
        # array for user
